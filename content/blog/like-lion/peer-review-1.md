...
단순히 포트폴리오용 프로젝트가 아닌 나의 코드 컨벤션을 확인할 수 있는 좋은 기회라고 생각한다.
...
> 개발을 완료하고, DB에 잘 들어가는지 체킹을 했지만, `@CreateDate`가 작동하지 않는 것을 확인했다.<br>
> 당시에는 무엇이 문제인지 제대로 인지하지 못하고, 결국 `@Builder`를 통해 Date를 직접 주입해주었다.<br>
