...
if (remains.isNotEmpty()) {
    throw ProfanityDetectedException(remains)
        ...
        conclusion: {
            outcomes: [
                'O(N×K) → O(N) 전환 및 JMH로 10만 자 텍스트 기준 정규식 대비 약 22배 성능 향상(70.64ms → 3.15ms)',
                '사이드 프로젝트가 무산된 후에도 라이브러리는 끝까지 완성해 Jitpack에 오픈소스로 배포',
                '변칙 비속어 탐지를 위한 정규화 파이프라인과 허용 단어 교차 검증 설계'
            ],
    ...
    },
    {
        ...
        overview: {
            description: '사내 인프라 기반의 AI 리뷰 시스템으로 팀 병목을 해소하고, 퇴사 후에도 구조적 한계를 직접 개선한 AI 코드 리뷰 엔진',
            techStack: [
...
fun startWorkers() {
    repeat(workerCount) { idx ->
        ...
        conclusion: {
            outcomes: [
                '리뷰 대기 시간을 최대 1시간 이상에서 5분 이내로 줄여 휴먼 리뷰어가 핵심 로직에만 집중할 수 있는 환경 조성',
                'QA 시 발견되는 백엔드 에러가 도입 전 대비 약 70% 감소해 사내 공식 성과로 인정',
                '예산 없이 사내 인프라만으로 구축해 외부망 노출 없이 보안 가이드라인을 준수'
            ],
...
pub async fn generate_with_fallback(prompt: String) -> Result<String, AppError> {
    ...
    Err(AppError::AiServiceUnavailable)
        ...
        conclusion: {
            outcomes: [
                'Harness Protocol 기반 에이전트 제어로 기획부터 배포까지 1주일 만에 완성(커밋 259+, 코드 9,400줄+)',
                'SSOT, Atomic Loop, Verification Gate 3단계 제약으로 AI 컨텍스트 오염을 차단 및 빌드 완료 확인 후 작업이 완료되도록 강제',
                '완료된 업무를 기반으로 성과 문서를 작성해 성과 복기 부담을 줄임'
            ],
