      ...
      },
      {
        ...
        point: {
          ...
          diagram: {
            type: 'fallback',
            steps: [
              { label: 'Local Cache', description: '기존 성과 데이터 조회' },
              { label: 'Gemini 2.0 Flash-Lite', description: '가장 빠르고 가벼운 모델 시도' },
              { label: 'Gemini 2.0 Flash', description: '표준 성능 모델로 재시도' },
              { label: 'Gemini 1.5 Pro', description: '최종 고성능 모델로 재시도' }
            ]
          },
          codeSnippet: `async function generateWithFallback(
  prompt: string
): Promise<string> {
  const models = [
  ...
  for (const modelName of models) {
    try {
      ...
      if (isRateLimitError(error)) {
        console.warn(\`Switching model: \${modelName} failed\`);
        continue; // 폴백 수행
      }
      throw error;
    }
    ...
    deepDives: [
      {
        ...
        point: {
          ...
          diagram: {
            type: 'pipeline',
            steps: [
              { label: 'Normalization', description: '숫자/공백 제거 및 소문자화' },
              { label: 'Trie Scan', description: 'Aho-Corasick 알고리즘 탐색' },
              { label: 'Interval Mapping', description: '탐지 구간 인덱스 매핑' }
            ]
          },
          codeSnippet: `fun findMatches(text: String): List<Match> = 
    normalize(text).let { normalized ->
      ...
      },
      {
        ...
        point: {
          ...
          diagram: {
            type: 'pipeline',
            steps: [
              { label: 'Matches', description: '비속어 탐지 구간 추출' },
              { label: 'Whitelist Scan', description: '허용 단어 구간 탐색' },
              { label: 'Overlap Check', description: '구간 포함 여부 검증' }
            ]
          },
